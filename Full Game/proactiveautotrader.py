    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            if self.position < 0:
                ...
                bid_volume = min(bid_volumes[0] + bid_volumes[1], 20)
                ...
                if self.bid_id == 0 and new_bid_price != 0 and self.position < 100 and (
                        self.future_position - 1 > -100):
                    self.bid_id = next(self.order_ids)
                    self.bid_price = new_bid_price
                ...
                ask_volume = min(ask_volumes[0] + ask_volumes[1], 20)
            ...
            # 100 difference between buy price
                ...
                if self.ask_id == 0 and new_ask_price != 0 and self.position > -100 and (
                        self.future_position + 1 < 100):
                    self.ask_id = next(self.order_ids)
                    self.ask_price = new_ask_price
