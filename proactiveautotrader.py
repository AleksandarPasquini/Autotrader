...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            ...
            new_ask_price = (new_bid_price - 100) if ask_prices[0] != 0 else 0
            bid_volume = 10
            ask_volume = 10
            ...
            if self.bid_id == 0 and new_bid_price != 0 and self.position < 100 - bid_volume and (
                    self.future_position - bid_volume > -100):
                self.bid_id = next(self.order_ids)
                self.bid_price = new_bid_price
                self.send_insert_order(self.bid_id, Side.BUY, new_bid_price, bid_volume, Lifespan.GOOD_FOR_DAY)

            if self.bid_id == 0 and new_bid_price != 0 and self.position <= 100 and (self.future_position - 1 >= -100):
                self.bid_id = next(self.order_ids)
                self.bid_price = new_bid_price
            ...
            if self.ask_id == 0 and new_ask_price != 0 and self.position > -100 + ask_volume and (
                    self.future_position + ask_volume < 100):
                self.ask_id = next(self.order_ids)
                self.ask_price = new_ask_price
                self.send_insert_order(self.ask_id, Side.SELL, new_ask_price, ask_volume, Lifespan.GOOD_FOR_DAY)

            if self.ask_id == 0 and new_ask_price != 0 and self.position >= -100 and (self.future_position + 1 <= 100):
                self.ask_id = next(self.order_ids)
                self.ask_price = new_ask_price
